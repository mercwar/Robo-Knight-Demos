...
size = (DWORD)(bufferChars * (DWORD)sizeof(wchar_t));
...
RKREG_API BOOL __stdcall WR_ReadDword(WR_HANDLE* handle, const wchar_t* name, DWORD* outValue)
{
    ...
    size = sizeof(DWORD);
    if (RegQueryValueExW(handle->hKey, name, NULL, &type, (BYTE*)outValue, &size) != ERROR_SUCCESS) return FALSE;
    return type == REG_DWORD;
}

/* PAGE: GF OBJECT (GF_ from gf.h or rkfio.h) */
...
RKREG_API BOOL __stdcall REG_GF_Init(GF_* gf, HKEY root, const wchar_t* subKey)
{
    ...
    /* CLEAN, WARNING-FREE ROOT KEY DETECTION */
    ...
    strncat(gf->url, subKeyA, GF_MAX_URL - 1 - (int)strlen(gf->url));
